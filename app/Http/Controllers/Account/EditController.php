...
class EditController extends AccountBaseController
{
	...
	public function updateDetails(UserRequest $request)
	{
        ...
        {
            if (file_exists(public_path('/uploads/user/'.$user->filename)) && $user->filename!="" )
            {
                unlink(public_path('/uploads/user/'.$user->filename));
            }

            $fileName = time().'_'.$request->file('filename')->getClientOriginalName();
            $request->file('filename')->move("uploads/user", $fileName);
